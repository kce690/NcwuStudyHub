...
def _build_mixed_preview(slides: list[dict], max_slides: int = 8) -> list[str]:
    ...
    return _build_mixed_preview_from_cleaned(cleaned, max_slides=max_slides)


def _build_mixed_preview_from_cleaned(cleaned: list[dict], max_slides: int = 8) -> list[str]:
    lines = ["## 图文混排速览"]
    picked = 0
...
def build_final_note(doc_title: str, slides: list[dict], ai_note: str | None) -> str:
    ...
    if "## 图文混排速览" not in note:
        mixed_preview = _build_mixed_preview_from_cleaned(cleaned)
        if mixed_preview:
            note += "\n\n" + "\n".join(mixed_preview)
